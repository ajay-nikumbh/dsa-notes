...
| Negatives mixed | `[-5, -1, -3]` | `1` | Doubling a negative makes it smaller, not larger |
...
# All identical positives never qualify
assert reversePairs([5, 5, 5]) == 0

# All identical negatives DO qualify: -5 > 2 * -5 = -10
assert reversePairs([-5, -5]) == 1

# Already sorted
assert reversePairs([1, 2, 3, 4, 5]) == 0

# Reverse sorted: not every inversion is a reverse pair
assert reversePairs([5, 4, 3, 2, 1][:]) == brute([5, 4, 3, 2, 1]) == 4

# Negatives mixed
assert reversePairs([-5, -1, -3][:]) == brute([-5, -1, -3]) == 1

# Zeroes
assert reversePairs([0, 0, 0]) == 0

# Extreme values: 2 * nums[j] would overflow 32-bit
