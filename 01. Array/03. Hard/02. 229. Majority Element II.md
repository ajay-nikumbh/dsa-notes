...
def majorityElement(nums: List[int]) -> List[int]:
    # First candidate slot, unassigned initially
    candidate1 = None
    # Second candidate slot, unassigned initially
    candidate2 = None
    # Vote count for candidate1
    count1 = 0
    # Vote count for candidate2
    ...
    # First pass: find possible candidates
    for num in nums:
        # Matches candidate1: add a vote
        if num == candidate1:
            count1 += 1
        # Matches candidate2: add a vote
        elif num == candidate2:
            count2 += 1
        # Candidate1 slot is free: claim it
        elif count1 == 0:
            candidate1 = num
            count1 = 1
        # Candidate2 slot is free: claim it
        elif count2 == 0:
            candidate2 = num
            count2 = 1
        # Neither slot matches and both are occupied: cancel one vote from each
        else:
            count1 -= 1
            count2 -= 1
    ...
    # Second pass: count actual occurrences of both candidates
    for num in nums:
        # Tally an occurrence of candidate1
        if num == candidate1:
            count1 += 1
        # Tally an occurrence of candidate2
        elif num == candidate2:
            count2 += 1

    # Collected majority elements
    result = []
    # Candidate1 qualifies only if strictly more than n // 3 occurrences
    if count1 > len(nums) // 3:
        result.append(candidate1)
    # Candidate2 qualifies only if strictly more than n // 3 occurrences
    if count2 > len(nums) // 3:
        result.append(candidate2)
    # Return the verified majority elements
    return result
...
def brute(nums: List[int]) -> List[int]:
    ...
    n = len(nums)
    # Count every value and keep those occurring more than n // 3 times
...
for case in ([1, 1, 1], [5, 5, 5, 5, 5], [7, 7, 8, 8]):
    # Compute the majority elements for this case
    out = majorityElement(case)
    # Length must match the size of the deduplicated set
    assert len(out) == len(set(out)), case

# Cross-check against brute force on random inputs
random.seed(229)
for _ in range(3000):
    # Random small array length
    n = random.randint(1, 14)
    # Random array with values in a small range
...
for _ in range(300):
    # Random larger array length
    n = random.randint(30, 120)
    # Random background values
    arr = [random.randint(-50, 50) for _ in range(n)]
    # Plant a guaranteed majority-ish value
    arr += [999] * (n // 2)
    # Shuffle so the planted value is not clustered
    random.shuffle(arr)
    # Optimal result must match the brute force reference
