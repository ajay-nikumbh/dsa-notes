...
# Optimal merge-sort based inversion counter
def inversionCount(arr: List[int]) -> int:
    ...
    # Recursively split, count each half, then count the merge
    def merge_sort(left_index: int, right_index: int) -> int:
        # If there is only one element, no inversion is possible
        ...
        # Find the middle index
        mid = (left_index + right_index) // 2
        # Total inversions is left + right + cross count
...
# Single element: recursion base case must return immediately
assert inversionCount([7]) == 0

# Two elements sorted, and two elements reversed
assert inversionCount([1, 2]) == 0
assert inversionCount([2, 1]) == 1

# Already sorted -> 0 (left branch always wins; count stays zero)
assert inversionCount(list(range(1, 51))) == 0

# Reverse sorted -> maximum n(n-1)/2
for n in range(1, 30):
    # Reverse-sorted array of size n must have the maximum possible inversions
    assert inversionCount(list(range(n, 0, -1))) == n * (n - 1) // 2

# All identical / duplicates mixed in must not inflate the count
assert inversionCount([2, 2, 1, 1]) == 4
assert inversionCount([1, 1, 1, 1]) == 0

# Negatives work too (only relative order matters)
...
for _ in range(400):
    ...
    # Optimal result must match the brute force reference (pass a copy: mutates input)
...
# Scales to the constraint size (large n fits O(n log n))
big = [random.randint(1, 10 ** 4) for _ in range(10 ** 5)]
# Large input must return a valid non-negative count without timing out
