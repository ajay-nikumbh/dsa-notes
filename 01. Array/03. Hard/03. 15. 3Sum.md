...
def brute(nums: List[int]) -> List[List[int]]:
    # Try every combination of three elements and keep those summing to zero
    seen = {tuple(sorted(t)) for t in combinations(nums, 3) if sum(t) == 0}
    # Return as a sorted list of lists for comparison
    return sorted(list(t) for t in seen)


def norm(triplets):
    # Normalize a list of triplets for order-independent comparison
...
# Mixed signs with a repeated anchor: the second -1 anchor must be skipped
assert norm(threeSum([-4, -1, -1, 0, 1, 2])) == [[-1, -1, 2], [-1, 0, 1]]

# Large magnitudes: values at the constraint boundary still sum exactly
...
for case in ([-1, 0, 1, 2, -1, -4], [0, 0, 0, 0], [-2, 0, 1, 1, 2], [3, 0, -2, -1, 1, 2]):
    # Compute the normalized triplets for this case
    out = norm(threeSum(case))
    # Number of triplets must equal the number of distinct triplets
...
for _ in range(1000):
    ...
    # Random array drawn from a small set with repeated values
