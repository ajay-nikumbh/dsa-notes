...
# Single element, match: needs the {0: 1} seed to be found at all
assert subarrayXor([6], 6) == 1
# Single element, no match: map lookup must miss cleanly
assert subarrayXor([5], 6) == 0
# Single zero element with k = 0
assert subarrayXor([0], 0) == 1

# k = 0 with all zeros: every subarray qualifies, n(n+1)/2
assert subarrayXor([0, 0, 0], 0) == 6
# Larger all-zero array, same n(n+1)/2 pattern
assert subarrayXor([0] * 10, 0) == 10 * 11 // 2

# All identical values
assert subarrayXor([7, 7, 7, 7], 7) == 6
# All identical values with k = 0: even-length subarrays XOR to zero
...
# Result must be a valid non-negative count and run without timing out
