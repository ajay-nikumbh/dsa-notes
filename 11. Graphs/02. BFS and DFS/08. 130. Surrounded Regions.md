...
# No 'O' cells at all: border BFS has nothing to seed, nothing to flip
assert solve([["X", "X"], ["X", "X"]]) == [["X", "X"], ["X", "X"]]

# All 'O' cells on the border (2x2, every cell is a border cell)
assert solve([["O", "O"], ["O", "O"]]) == [["O", "O"], ["O", "O"]]

# Single row: every cell is a border cell, nothing can be surrounded
assert solve([["O", "O", "O"]]) == [["O", "O", "O"]]

# Single column: same reasoning, no interior cells exist
