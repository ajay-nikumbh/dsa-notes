...
# Single one, hit: smallest possible window must still count
assert numSubarraysWithSum([1], 1) == 1
# Single one, miss: atMost(-1) guard must fire
assert numSubarraysWithSum([1], 0) == 0
# Single zero: a zero-only subarray is a valid answer
assert numSubarraysWithSum([0], 0) == 1

# Zeroes around ones: counts runs of zeroes, [0],[0],[0],[0,0]
assert numSubarraysWithSum([0, 1, 0, 0], 0) == 4

# All ones: windows of fixed length, shrink runs every step
assert numSubarraysWithSum([1, 1, 1], 2) == 2
assert numSubarraysWithSum([1, 1, 1], 3) == 1

# Goal exceeds total ones: both at-most counts equal, difference is 0
assert numSubarraysWithSum([1, 0, 1], 3) == 0

# All ones, goal = 0: no zero-sum subarray exists
assert numSubarraysWithSum([1, 1, 1], 0) == 0

# All zeroes, goal = 0: n(n+1)/2, the classic guard test
for n in range(1, 40):
