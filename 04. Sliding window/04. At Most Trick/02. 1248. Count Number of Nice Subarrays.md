...
# Sparse odds: padding evens multiply into the count at scale
...
for _ in range(1000):
    ...
    # Values between 1 and 9 give a mix of odd and even numbers
    arr = [random.randint(1, 9) for _ in range(n)]
    # Target odd-count within the array length
    kk = random.randint(1, n)
    # Answer must match LC 930's solution on the odd/even-mapped array
