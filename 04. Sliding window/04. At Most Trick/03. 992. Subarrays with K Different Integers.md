...
def subarraysWithKDistinct(nums: List[int], k: int) -> int:
    def at_most(max_distinct: int) -> int:
        ...
        if max_distinct < 0:
            return 0
        # Frequency of each number inside the current window
        frequency = {}
        # Left edge of the window
        left = 0
        # Number of subarrays with at most max_distinct distinct values
        count = 0
        # Expand the window one element at a time
        for right in range(len(nums)):
            # Add the incoming element to the frequency map
            frequency[nums[right]] = frequency.get(nums[right], 0) + 1
            # Shrink from the left while too many distinct values are in the window
            while len(frequency) > max_distinct:
                # Drop one occurrence of the leftmost element
                frequency[nums[left]] -= 1
                # Remove it from the map entirely once its count hits zero
                if frequency[nums[left]] == 0:
                    del frequency[nums[left]]
                left += 1
...
def brute(nums: List[int], k: int) -> int:
    ...
    n = len(nums)
    # Count of subarrays with exactly k distinct values
    total = 0
    # Try every possible starting index
    for start in range(n):
        # Distinct values seen in the current subarray
        seen = set()
        for end in range(start, n):
            # Extend the subarray by one element
            seen.add(nums[end])
            # Count it if the distinct count matches the target exactly
            if len(seen) == k:
                total += 1
            # Stop early once distinct count exceeds the target
            elif len(seen) > k:
                break
    return total
...
# Single element, k = 1: smallest good subarray
assert subarraysWithKDistinct([1], 1) == 1

# All identical, k = 1: every subarray qualifies (n(n+1)/2)
assert subarraysWithKDistinct([5, 5, 5], 1) == 6
# All identical, k = 2: only one distinct value exists
assert subarraysWithKDistinct([5, 5, 5], 2) == 0

# All distinct, k = 1: only the singletons
assert subarraysWithKDistinct([1, 2, 3], 1) == 3
# All distinct, k = n: only the whole array
assert subarraysWithKDistinct([1, 2, 3], 3) == 1

# k exceeds distinct count: both at-most counts saturate equally
assert subarraysWithKDistinct([1, 2, 1, 2], 3) == 0

# Structural properties across sizes
for n in range(1, 30):
    # Array of n distinct values
    arr = list(range(n))
    # Only the whole array holds all n distinct values
    assert subarraysWithKDistinct(arr, n) == 1
    # Only the n singleton subarrays hold exactly one distinct value
...
random.seed(992)
for _ in range(5000):
    # Random small array and target distinct-count
    n = random.randint(1, 12)
    # Small alphabet so repeats inside the window are common
    arr = [random.randint(1, 4) for _ in range(n)]
    # Target distinct-count within the array length
...
for _ in range(2000):
    ...
    # Wide alphabet so distinct count exceeds k frequently
...
for _ in range(300):
    ...
    # Small alphabet so distinct counts span a useful range
    arr = [random.randint(1, 6) for _ in range(n)]
    # Summing the count for every possible k covers every subarray exactly once
