...
def min_window(s: str, t: str) -> str:
    # If either string is empty, no window can exist
    if not s or not t:
        return ""
    # If t is longer than s, no window can exist
    if len(t) > len(s):
        return ""
    ...
    # Store how many distinct characters must be fully satisfied
    required = len(target_count)
    # Store frequency of characters in the current window
    window_count: Dict[str, int] = {}
    # Store how many distinct characters are currently satisfied
    formed = 0
    # Store the left boundary of the window
    left = 0
    # Store the length of the smallest valid window found so far
    min_length = float("inf")
    # Store the starting index of the smallest valid window
    min_start = 0

    # Grow the window one character at a time
    for right in range(len(s)):
        # Read the character entering the window
        right_char = s[right]
        # Add it into the window frequency map
        window_count[right_char] = window_count.get(right_char, 0) + 1
        ...
        while formed == required:
            ...
            # Read the character leaving the window
            left_char = s[left]
            # Remove it from the window frequency map
            window_count[left_char] -= 1
    ...
    if min_length == float("inf"):
        return ""
    # Return the smallest valid window as a substring
    return s[min_start:min_start + min_length]


# Define the author's single hash-map + counter formulation
def single_map_version(s: str, t: str) -> str:
    """The author's hash + cnt formulation."""
    # If either string is empty or t is longer than s, no window exists
    if not s or not t or len(t) > len(s):
        return ""
    # Store the required (and remaining) frequency of each character in t
    ...
    # Store the left boundary of the window
    left = 0
    # Store how many characters of t are currently satisfied (with multiplicity)
    cnt = 0
    # Store the length of the smallest valid window found so far
    min_length = float("inf")
    # Store the starting index of the smallest valid window
    ...
    # Grow the window one character at a time
    for right in range(len(s)):
        # If this character is still needed, count it as satisfied
        if hash_map.get(s[right], 0) > 0:
            cnt += 1
        # Decrement its remaining need (can go negative for surplus copies)
        hash_map[s[right]] = hash_map.get(s[right], 0) - 1

        # Shrink the window while it still fully covers t
        while cnt == len(t):
            # Update the best answer if this window is smaller
            if right - left + 1 < min_length:
                min_length = right - left + 1
                s_index = left
            # Restore the leaving character's remaining need
            hash_map[s[left]] += 1
            # If it is now needed again, the window is no longer fully valid
            if hash_map[s[left]] > 0:
                cnt -= 1
            # Advance the left boundary
            left += 1

    # Return empty string if no valid window was found, else the window
...
def brute_force(s: str, t: str) -> str:
    ...
    # Import Counter for frequency comparisons
    from collections import Counter
    # Store the required frequency of each character in t
    need = Counter(t)
    # Store the best (shortest) valid substring found so far
    best = ""
    # Try every possible starting index
    for start in range(len(s)):
        ...
        for end in range(start, len(s)):
            ...
            window[s[end]] += 1
            # If the window covers every required character, record it
            if all(window[c] >= need[c] for c in need):
                # Take the current substring as a candidate answer
                candidate = s[start:end + 1]
                # Update best if this candidate is shorter (or first found)
                if best == "" or len(candidate) < len(best):
                    best = candidate
                # Extending this start only makes it longer, so stop here
                break
    # Return the shortest valid substring found
    return best


# Define a helper to check whether a window satisfies t's character counts
def is_valid_window(window: str, t: str) -> bool:
    # Import Counter for frequency comparisons
    from collections import Counter
    # Count characters in the window and in the target
    wc, need = Counter(window), Counter(t)
    # The window is valid if it covers every required character count
...
for text, target in [
    ...
    # Both implementations must return the exact same window on each case
...
# Import product to generate every possible string over the alphabet
from itertools import product

# Two-letter alphabet, exhaustive over short s and t lengths
for s_len in range(1, 8):
    # Generate every possible s of this length over "ab"
    for s_tup in product("ab", repeat=s_len):
        text = "".join(s_tup)
        for t_len in range(1, 4):
            # Generate every possible t of this length over "ab"
            for t_tup in product("ab", repeat=t_len):
                target = "".join(t_tup)
                # Compute the ground-truth minimum length via brute force
                expected_len = len(brute_force(text, target))
                got = min_window(text, target)
                got_single = single_map_version(text, target)
                # Both fast solutions must match the brute force length
                assert len(got) == expected_len, (text, target, got)
                assert len(got_single) == expected_len, (text, target, got_single)
                # Whenever a window is found, it must actually be valid
                if got:
...
for s_len in range(1, 6):
    # Generate every possible s of this length over "abc"
    for s_tup in product("abc", repeat=s_len):
        text = "".join(s_tup)
        for t_len in range(1, 4):
            # Generate every possible t of this length over "abc"
            for t_tup in product("abc", repeat=t_len):
                target = "".join(t_tup)
                # Compute the ground-truth minimum length via brute force
                expected_len = len(brute_force(text, target))
                got = min_window(text, target)
                # The fast solution must match the brute force length
                assert len(got) == expected_len, (text, target, got)
                # Whenever a window is found, it must actually be valid
                if got:
                    assert is_valid_window(got, target), (text, target, got)
...
# Import random for generating skewed test inputs
import random

random.seed(19)
for _ in range(3000):
    # Pick random lengths and a skewed alphabet for s and t
    ...
    # Edge case: empty target must always return empty string
    if not target:
...
random.seed(20)
for _ in range(500):
    # Build a random text and a random target
    ...
    got = min_window(text, target)
    if got:
        # The returned window must be an actual substring of text
        assert got in text, (text, target, got)
        # The returned window must satisfy the character-count requirement
        assert is_valid_window(got, target), (text, target, got)
