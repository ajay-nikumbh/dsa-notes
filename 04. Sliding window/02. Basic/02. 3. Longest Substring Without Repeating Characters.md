...
def length_of_longest_substring(s: str) -> int:
    # Map each character to the index it was last seen at
    ...
    for right, current_char in enumerate(s):
        if current_char in last_seen:
            # Move left forward only, never backward.
            left = max(left, last_seen[current_char] + 1)
        # Record the current character's latest index
        last_seen[current_char] = right
        # Update the best window length seen so far
        max_length = max(max_length, right - left + 1)
    # Return the longest duplicate-free window length
    return max_length
...
def brute_force(s: str) -> int:
    # Longest duplicate-free substring length found so far
    best = 0
    # Try every possible start index
    for start in range(len(s)):
        # Characters seen in the current substring
        seen = set()
        # Extend the substring one character at a time
        for end in range(start, len(s)):
            # Stop extending once a duplicate is found
            if s[end] in seen:
                break
            seen.add(s[end])
            # Update the best length seen so far
            best = max(best, end - start + 1)
    # Return the brute force best length
    return best


import random

# Fix the seed for reproducible random strings
random.seed(3)
for _ in range(3000):
    # Random string length
    n = random.randint(0, 14)
    # Random alphabet to control how many repeats occur
    ...
    # Hashmap-jump solution must match the brute force reference
    assert length_of_longest_substring(text) == expected, text
    # Set-based solution must match the brute force reference too
...
def best_window(s: str):
    ...
    # Longest window length found so far
    best = 0
    # Start index of the best window found so far
    best_start = 0
    for right, ch in enumerate(s):
        if ch in last_seen:
            # Move left forward only, never backward
        ...
        # Record the current character's latest index
        last_seen[ch] = right
        # Update the best window if this one is longer
        if right - left + 1 > best:
            best = right - left + 1
            best_start = left
    # Return the actual best window substring
    return s[best_start:best_start + best]


for text in ["abcabcbb", "bbbbb", "pwwkew", "abba", "cadbzabcd", "aab", ""]:
    # Compute the best window substring for this case
    window = best_window(text)
    # The window itself must contain no duplicate characters
    assert len(window) == len(set(window)), (text, window)
    # Its length must match the reported optimal length
    assert len(window) == length_of_longest_substring(text), (text, window)

# Performance sanity at the constraint bound
big = "abcdefghij" * 5000
# Large repeating input must still return the correct answer quickly
