...
def min_subarray_len_with_if(target, nums):
    ...
    for right in range(len(nums)):
        # Add the current element into the window sum.
        s += nums[right]
        # Only a single shrink attempt per right, instead of shrinking while valid.
        if s >= target:
            # Update minimum length.
...
for _ in range(800):
    ...
    # Get the reported minimum length.
    k = minSubArrayLen(t, arr)
    # If no valid subarray was reported, total sum must fall short of target.
    if k == 0:
        assert sum(arr) < t, (t, arr)
    else:
        # Some window of length k must actually reach the target.
        assert any(sum(arr[i:i + k]) >= t for i in range(n - k + 1)), (t, arr, k)
        # No window one shorter than k may reach the target.
        assert all(sum(arr[i:i + k - 1]) < t for i in range(n - k + 2)), (t, arr, k)

# Large input stays linear and correct
big = [1] * 100000
# Target equal to array length: whole array is exactly enough.
assert minSubArrayLen(100000, big) == 100000
# Target one more than total sum: no valid subarray exists.
