...
def number_of_substrings(s: str) -> int:
    ...
    n = len(s)
    # Frequency of each required character inside the current window
    freq: Dict[str, int] = {"a": 0, "b": 0, "c": 0}
    # Left edge of the window
    left = 0
    # Number of valid substrings found so far
    count = 0
    # Expand the window one character at a time
    for right in range(n):
        # Track the incoming character
        freq[s[right]] += 1
        # While the window already contains all three characters
        while freq["a"] > 0 and freq["b"] > 0 and freq["c"] > 0:
            # Window s[left:right+1] is valid, so every extension is too.
            count += n - right
            # Drop the leftmost character and shrink the window
            freq[s[left]] -= 1
            left += 1
    return count
...
def last_seen_version(s: str) -> int:
    # Most recent index at which each character was seen
    last_seen = {"a": -1, "b": -1, "c": -1}
    # Number of valid substrings found so far
    count = 0
    # Walk the string, updating the last-seen index for each character
    for right, ch in enumerate(s):
        last_seen[ch] = right
        # Earliest last-seen index among the three required characters
        earliest = min(last_seen["a"], last_seen["b"], last_seen["c"])
        # Once all three have appeared, every start up to earliest works
        if earliest != -1:
            count += earliest + 1
    return count
...
def brute_force(s: str) -> int:
    # Size of the input string
    n = len(s)
    # Count of substrings containing all three characters
    total = 0
    # Try every possible starting index
    for start in range(n):
        # Frequency of each required character in the current substring
        freq = {"a": 0, "b": 0, "c": 0}
        for end in range(start, n):
            # Extend the substring by one character
            freq[s[end]] += 1
            # Count it if all three characters are present
            if freq["a"] > 0 and freq["b"] > 0 and freq["c"] > 0:
                total += 1
    return total
...
for length in range(1, 10):
    ...
    for tup in product("abc", repeat=length):
        text = "".join(tup)
        # Reference answer from the brute force check
...
random.seed(7)
for _ in range(2000):
    # Random length up to 40
    n = random.randint(0, 40)
    # Pick an alphabet, some skewed so one character dominates
    alphabet = random.choice(["abc", "aab", "aaabc", "ab", "c"])
    # Build a random string from the chosen alphabet
...
for repeats in range(1, 12):
    # Repeat "abc" to build the test string
    text = "abc" * repeats
    n = len(text)
...
big = "abc" * 16666 + "ab"
# Both optimal versions must agree on a large input
assert number_of_substrings(big) == last_seen_version(big)
# Answer must exceed a 32-bit signed int's range
