...
# Single character, k = 0: smallest window; the loop must still record an answer
assert character_replacement("A", 0) == 1

# No budget at all: with k = 0 the answer is the longest run of one letter
assert character_replacement("ABAB", 0) == 1
# k = 0, long run: must find "BBB" and not be fooled by total counts
assert character_replacement("AABBBCC", 0) == 3
assert character_replacement("AAAA", 0) == 4

# All identical: cost is always 0; window never shrinks
assert character_replacement("AAAA", 2) == 4

# Budget covers everything: k >= n - 1 always yields n
assert character_replacement("ABCD", 3) == 4

# Budget exceeds need: answer is capped by len(s), never by k
assert character_replacement("AB", 5) == 2
assert character_replacement("A", 100) == 1

# Answer is the whole string: window must be allowed to reach full length
assert character_replacement("AABA", 1) == 4

# Two-letter tie: majority is B; replacing the single A costs 1
...
# Two-letter alphabet, exhaustive over lengths 1-8 and every k
for length in range(1, 9):
    # Exhaustively generate every string of this length over {A, B}
    for tup in product("AB", repeat=length):
        text = "".join(tup)
        # Try every possible replacement budget for this string
        for k in range(0, length + 1):
            # Compute the trusted answer via brute force
...
for length in range(1, 7):
    # Exhaustively generate every string of this length over {A, B, C}
...
random.seed(12)
for _ in range(200):
    # Random string over a three-letter alphabet
    text = "".join(random.choice("ABC") for _ in range(random.randint(1, 30)))
    # Best answer seen so far as k increases
    previous = 0
    for k in range(0, len(text) + 1):
        # Answer must never decrease as the budget k grows
...
big = "AABABBA" * 14285
# Both variants must agree on a large input
assert character_replacement(big, 1) == lazy_if_version(big, 1)
# A budget covering the whole string always yields the whole string
