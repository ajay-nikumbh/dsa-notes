...
# Sort boards so solution sets can be compared regardless of order
def normalize(solutions):
    return sorted(tuple(board) for board in solutions)


# Trivial board: n = 1, single cell, no constraints to violate
assert solve_n_queens(1) == [["Q"]]

# No solution (small board): n = 2 and n = 3 are too tight for mutually safe queens
assert solve_n_queens(2) == []
assert solve_n_queens(3) == []

# Smallest board with solutions: n = 4 matches the textbook example (order-independent compare)
...
# Solution counts against known values: n = 4 -> 2 solutions, larger board n = 8 -> 92 solutions
...
for n in range(1, 9):
    for board in solve_n_queens(n):
        # Board has exactly n rows
        assert len(board) == n
        # Extract the (row, col) position of the queen in each row
