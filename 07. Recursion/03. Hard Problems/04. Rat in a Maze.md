...
n = len(maze4)
# Replay each returned path move-by-move to verify it is a genuine, collision-free route
for p in paths:
    r, c = 0, 0
    seen = {(0, 0)}
    for move in p:
        # Apply the move's delta
        dr, dc = deltas[move]
        r, c = r + dr, c + dc
        # Every intermediate cell must stay inside the grid
        assert 0 <= r < n and 0 <= c < n
        # Every intermediate cell must be open
        assert maze4[r][c] == 1
        # No cell may be revisited within the same path
        assert (r, c) not in seen
        seen.add((r, c))
    # The path must end exactly at the destination
    assert (r, c) == (n - 1, n - 1)
