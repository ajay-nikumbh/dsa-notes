...
# Target unreachable: every candidate exceeds target, pick branch never fires
assert as_set(combination_sum([2], 1)) == []

# Edge cases
# Single candidate equals target: minimal valid pick path
assert as_set(combination_sum([7], 7)) == [[7]]
# Candidate larger than target: guard must reject immediately
assert as_set(combination_sum([2, 3, 6, 7], 1)) == []
# Target requires many repeats: confirms unlimited reuse at the same index
assert as_set(combination_sum([2], 8)) == [[2, 2, 2, 2]]
# Target equals smallest candidate exactly: base case fires on the first pick
