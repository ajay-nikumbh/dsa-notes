...
# Normalize a list of combinations for order-independent comparison
def as_set(combos):
...
# Duplicates required in output: two 2s from different indices legitimately combine
assert as_set(combination_sum2([2, 5, 2, 1, 2], 5)) == as_set([[1, 2, 2], [5]])

# Edge cases
# Duplicate candidates fully consumed: uses both 1s at different recursion depths
assert as_set(combination_sum2([1, 1, 2], 4)) == [[1, 1, 2]]
# Target unreachable: no candidate small enough to start
assert as_set(combination_sum2([5, 10], 3)) == []
# Candidate larger than target: pruned immediately by the break
assert as_set(combination_sum2([10], 5)) == []
# Single candidate equals target: minimal valid path
assert as_set(combination_sum2([6], 6)) == [[6]]
# All identical candidates: dedup must not also block different indices producing [2,2] once
