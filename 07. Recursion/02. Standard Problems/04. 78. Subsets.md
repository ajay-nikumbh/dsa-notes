...
def as_set(subsets: List[List[int]]):
    # Build a comparison-friendly set representation of the subsets.
...
for nums in [[1], [1, 2], [1, 2, 3], [4, -1, 7, 2]]:
    # Compute the subsets for this input.
    result = sol.subsets(nums)
    # Confirm the total count matches 2^N.
    assert len(result) == 2 ** len(nums)
    # Confirm the empty subset is always present.
    assert [] in result
    # Confirm the full set itself is always present.
    assert sorted(nums) in [sorted(s) for s in result]

# Maximum length: N = 10 -> checks the exponential blow-up is handled
big = list(range(1, 11))
assert len(sol.subsets(big)) == 1024

# Deliberately test the classic "appended a reference not a copy" bug
class BuggySolution:
    # Same backtracking skeleton, but without copying current before storing it.
    def subsets(self, nums: List[int]) -> List[List[int]]:
        # Create the final list containing every subset.
        answer = []
        # Create the subset currently being constructed.
        ...
        # Begin with no elements chosen.
        backtrack(0)
        # Return every subset (all aliasing the same list object).
        return answer
