...
# Normalize a list of subsets so that equal sets compare equal regardless of order.
...
# Create one Solution instance to reuse across all assertions.
sol = Solution()
...
# Expected subset count is (multiplicity + 1) multiplied across distinct values.
expected_count = (3 + 1) * (1 + 1)  # 8
assert len(result) == expected_count
# Confirm none of those subsets are duplicates of each other.
seen = [tuple(sorted(s)) for s in result]
assert len(seen) == len(set(seen))

# Unsorted input with duplicates: confirms sorting happens inside the function itself
